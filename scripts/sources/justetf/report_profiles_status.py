...
def summarize_progress(progress: list[dict]) -> dict:
...
def display_summary(run_dir: Path, counts: dict, progress: list[dict]) -> None:
    ...
    for key in ["ok", "skip", "err", "total"]:
        table.add_row(key.upper(), str(counts[key]))
    ...
    if counts["err"] > 0:
        ...
        for f in err_files:
            try:
                ...
                console.print(f"- [red]{data.get('isin')}[/red]: {data.get('error')}")
            except Exception:
                console.print(f"- [red]{f.name}[/red] (unreadable error file)")
        if len(err_files) < counts["err"]:
            console.print(f"... and {counts['err'] - len(err_files)} more\n")
    else:
